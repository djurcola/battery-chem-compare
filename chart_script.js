...
// Note: This function is still used for the *initial* chart creation.
...
async function initializeChart() {
    ...
    try {
        ...
        console.log("Populating categories...");
         ...
         document.getElementById('category-selector')?.addEventListener('change', handleCategoryChange); // Attaching the *corrected* handler
...
// --- Handle Category Change --- <<<< ****** THIS FUNCTION IS THE CORRECTED VERSION ****** >>>>
function handleCategoryChange(event) {
    if (!batteryChart || !allPreNormalizedData?.length || !loadedBatteryData?.length) {
        console.warn("Chart/Data not ready for category change.");
        return;
    }
    ...
    if (newVisibleKeys.length < 3) {
        // Re-check the checkbox that was just unchecked, if possible
        if (event?.target instanceof HTMLInputElement && !event.target.checked) {
            event.target.checked = true;
        }
        alert("Please select at least 3 categories to compare.");
        return; // Prevent update if validation fails
    }

    console.log("Category changed. New visible keys:", newVisibleKeys);

    // 1. Get the new labels for the axes
    const newVisibleLabels = newVisibleKeys.map(key => metricMetadata[key]?.label || key);

    // 2. Get the indices corresponding to the new keys within the *full* data array
    const visibleIndices = newVisibleKeys.map(key => allMetricKeys.indexOf(key)).filter(index => index !== -1);

    // 3. Update the data points within each *existing* dataset
    let updateError = false;
    try {
        if (batteryChart.data.datasets.length !== allPreNormalizedData.length) {
             throw new Error("Mismatch between chart datasets and pre-normalized data length.");
        }

        batteryChart.data.datasets.forEach((dataset, index) => {
            // Find the corresponding full data row from the pre-calculated data
            const fullDataRow = allPreNormalizedData[index];
            if (!fullDataRow) {
                console.warn(`No pre-normalized data found for dataset index ${index}`);
                // Keep existing data points for this dataset to avoid errors,
                // though this indicates a potential data inconsistency.
                return;
            }

            // Extract the data points corresponding to the *new* visible keys
            const newVisibleDataPoints = visibleIndices.map(idx => fullDataRow[idx]);

            // --- This is the key change: Update the data property directly ---
            dataset.data = newVisibleDataPoints;
            // -----------------------------------------------------------------
        });
    } catch (error) {
        console.error("Error updating dataset data:", error);
        showError("Error updating chart with new categories.");
        updateError = true;
    }

    if (updateError) return; // Don't proceed if data update failed

    // 4. Update the chart's labels (axes)
    batteryChart.data.labels = newVisibleLabels;

    // 5. Update the chart to reflect the changes
    console.log("Updating chart with new category data...");
    batteryChart.update(); // Use 'none' animation if preferred: batteryChart.update('none');
...
// --- Resize Handler (Revised Logic) --- <<< ****** THIS IS THE ORIGINAL RESIZE HANDLER ****** >>>>
let resizeTimeout;
     ...
     resizeTimeout = setTimeout(() => {
         if (batteryChart) {
             try {
                 ...
                 let optionsChanged = false; // Track if specific options impacting redraw are changed
                 ...
                 // 2. Update Color-related options
                 ...
                 // Update other theme-dependent colors similarly
                 if (batteryChart.options.scales?.r?.pointLabels?.color !== newOptions.scales?.r?.pointLabels?.color) {
                     batteryChart.options.scales.r.pointLabels.color = newOptions.scales.r.pointLabels.color;
                     optionsChanged = true;
                 }
                 if (batteryChart.options.scales?.r?.grid?.color !== newOptions.scales?.r?.grid?.color) {
                     batteryChart.options.scales.r.grid.color = newOptions.scales.r.grid.color;
                     optionsChanged = true;
                 }
                  if (batteryChart.options.scales?.r?.angleLines?.color !== newOptions.scales?.r?.angleLines?.color) {
                     batteryChart.options.scales.r.angleLines.color = newOptions.scales.r.angleLines.color;
                     optionsChanged = true;
                 }
                 if (batteryChart.options.plugins?.legend?.labels?.color !== newOptions.plugins?.legend?.labels?.color) {
                     batteryChart.options.plugins.legend.labels.color = newOptions.plugins.legend.labels.color;
                     optionsChanged = true;
                 }
                 // Tooltip colors (Note: These might not force a redraw on resize alone, but good practice to update)
                 if (batteryChart.options.plugins?.tooltip?.backgroundColor !== newOptions.plugins?.tooltip?.backgroundColor) {
                    batteryChart.options.plugins.tooltip.backgroundColor = newOptions.plugins.tooltip.backgroundColor;
                    optionsChanged = true; // Less likely to cause visual change without interaction, but track it
                 }
                 if (batteryChart.options.plugins?.tooltip?.titleColor !== newOptions.plugins?.tooltip?.titleColor) {
                     batteryChart.options.plugins.tooltip.titleColor = newOptions.plugins.tooltip.titleColor;
                     optionsChanged = true;
                 }
                 if (batteryChart.options.plugins?.tooltip?.bodyColor !== newOptions.plugins?.tooltip?.bodyColor) {
                     batteryChart.options.plugins.tooltip.bodyColor = newOptions.plugins.tooltip.bodyColor;
                     optionsChanged = true;
                 }


                // 3. Update Font sizes / Point sizes / Padding if they differ based on 'isMobile'
                if (batteryChart.options.scales?.r?.ticks?.font?.size !== newOptions.scales?.r?.ticks?.font?.size) {
                 ...
                 if (batteryChart.options.scales?.r?.pointLabels?.font?.size !== newOptions.scales?.r?.pointLabels?.font?.size) {
                     batteryChart.options.scales.r.pointLabels.font.size = newOptions.scales.r.pointLabels.font.size;
                     optionsChanged = true;
                 }
                 if (batteryChart.options.plugins?.legend?.labels?.font?.size !== newOptions.plugins?.legend?.labels?.font?.size) {
                     batteryChart.options.plugins.legend.labels.font.size = newOptions.plugins.legend.labels.font.size;
                     optionsChanged = true;
                 }
                 if (batteryChart.options.plugins?.legend?.labels?.boxWidth !== newOptions.plugins?.legend?.labels?.boxWidth) {
                     batteryChart.options.plugins.legend.labels.boxWidth = newOptions.plugins.legend.labels.boxWidth;
                     optionsChanged = true;
                 }
                  if (batteryChart.options.plugins?.legend?.labels?.padding !== newOptions.plugins?.legend?.labels?.padding) {
                     batteryChart.options.plugins.legend.labels.padding = newOptions.plugins.legend.labels.padding;
                     optionsChanged = true;
                 }
                 if (batteryChart.options.elements?.point?.radius !== newOptions.elements?.point?.radius) {
                     batteryChart.options.elements.point.radius = newOptions.elements.point.radius;
                     optionsChanged = true;
                 }
                 if (batteryChart.options.elements?.point?.hoverRadius !== newOptions.elements?.point?.hoverRadius) {
                     batteryChart.options.elements.point.hoverRadius = newOptions.elements.point.hoverRadius;
                     optionsChanged = true;
                 }
                 // Optional: Update hitRadius if needed, though less likely to affect visual redraw
                 // if (batteryChart.options.elements?.point?.hitRadius !== newOptions.elements?.point?.hitRadius) {
                 //    batteryChart.options.elements.point.hitRadius = newOptions.elements.point.hitRadius;
                 //    optionsChanged = true; // Track if you want potential update
                 // }
                 ...
                 // Optional: Force an update *after* resize if direct option changes aren't always picked up by resize()
                 // Especially if only colors/fonts changed, resize() might not redraw everything.
                 // if (optionsChanged) {
                 //     console.log("Forcing update('none') after resize due to option changes.");
                 //     batteryChart.update('none'); // 'none' prevents animation
                 // }
